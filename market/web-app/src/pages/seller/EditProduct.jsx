...
export default function EditProduct() {
  ...
  const loadProduct = async () => {
    try {
      ...
      // Validar propiedad
      if (prod.sellerUserId !== user.uid) {
        addToast('Acceso denegado. No eres el propietario.', 'error');
        navigate('/seller/products');
        return;
      }
      ...
      setFormData({
        ...
        images: prod.images ? prod.images.join(', ') : (prod.image ? prod.image : ''),
        allowAffiliates: prod.allowAffiliates || false,
        commissionRate: prod.commissionRate !== undefined ? prod.commissionRate.toString() : '10'
      });
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const updates = {
        ...
        image: formData.images ? formData.images.split(',')[0].trim() : '',
    ...
    <div className="min-h-screen bg-gray-50 pb-20">
      <div className="bg-white border-b border-gray-100 p-4 sticky top-0 z-30 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-bold text-xl text-gray-900 leading-tight">Editar Producto</h1>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto w-full p-4">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input required type="text" name="title" value={formData.title} onChange={handleChange} className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Descripción corta</label>
                <input type="text" name="shortDescription" value={formData.shortDescription} onChange={handleChange} className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Descripción completa</label>
                <textarea rows="4" name="description" value={formData.description} onChange={handleChange} className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none resize-none" />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" name="images" value={formData.images} onChange={handleChange} className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none" />
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-4 pt-4">
            ...
            <button type="submit" disabled={saving} className="flex-[2] bg-primary text-white font-bold py-4 rounded-xl shadow-lg shadow-primary/30 hover:bg-primary-variant transition-colors flex items-center justify-center">
              {saving ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Actualizar Producto'}
            </button>
          </div>
