...
export interface JemimaInventoryItem {
    id: string;
    productId: string;
    productName: string;
    sku: string;
...
class JemimaInventoryService {
    private collectionName = 'jemimaInventory';

    async getAll(): Promise<JemimaInventoryItem[]> {
    ...
    async markAsReturned(id: string): Promise<void> {
        await updateDoc(doc(db, this.collectionName, id), { status: 'devuelto' });
    }
...
export const jemimaInventoryService = new JemimaInventoryService();
