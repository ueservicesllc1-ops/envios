...
const AppVilma: React.FC = () => {
    const navigate = useNavigate();
    const { user, loading: authLoading } = useAnonymousAuth();

    const [loading, setLoading] = useState(true);
    const [allItems, setAllItems] = useState<VilmaInventoryItem[]>([]);
    ...
    // Pagos
    const [payments, setPayments] = useState<VilmaPayment[]>([]);
    ...
    // Edición de precio
    const [editingItem, setEditingItem] = useState<VilmaInventoryItem | null>(null);
    ...
    const loadData = useCallback(async () => {
        try {
            ...
            const [items, pays] = await Promise.all([
                vilmaInventoryService.getAll(),
                vilmaPaymentService.getAll()
            ]);
            setAllItems(items);
    ...
    const handleMarkAsSold = async (item: VilmaInventoryItem) => {
        try {
            await vilmaInventoryService.markAsSold(item.id);
    ...
    const handleReturn = async (item: VilmaInventoryItem) => {
        try {
            ...
            // Marcar como devuelto en vilmaInventory
            await vilmaInventoryService.markAsReturned(item.id);
    ...
    const handlePay = async () => {
        ...
        setPaying(true);
        try {
            await vilmaPaymentService.addPayment(amount);
    ...
    const handleUpdatePrice = async () => {
        ...
        try {
            // 1. Actualizar inventario de Vilma
            await vilmaInventoryService.updatePrice(editingItem.id, price, editingItem.quantity);

            // 2. Actualizar notas de salida para que los reportes sean correctos
            const allNotes = await exitNoteService.getAll();
            const vilmaNotes = allNotes.filter(n =>
                n.sellerId === 'vilma' &&
    ...
    const ProductCard = ({ item, showSoldButton, showReturnButton }: { item: VilmaInventoryItem; showSoldButton?: boolean; showReturnButton?: boolean }) => (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
            ...
            >
                ...
                <div className="absolute top-2 right-2 bg-pink-600 text-white text-xs font-bold px-2 py-1 rounded-full shadow">
                    x{item.quantity}
                </div>
            </div>
            ...
            <div className="p-3 flex flex-col flex-1">
                ...
                <div className="mt-auto pt-2 border-t border-gray-50 flex justify-between items-center">
                    <div className="flex flex-col">
                        ...
                        <span className="text-sm font-bold text-pink-600">${item.unitPrice.toFixed(2)}</span>
                    </div>
                    <button
                        onClick={() => {
                            setEditingItem(item);
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div className="p-4">
                        <button
                            ...
                            className="w-full bg-pink-600 hover:bg-pink-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-bold py-3 rounded-xl flex items-center justify-center space-x-2 transition-all active:scale-95"
                        >
                        ...
                        <button onClick={() => { toast.promise(generateSellerAppInventoryPDF("Vilma Uchubanda", inventoryItems), { loading: 'Generando PDF con im�genes...', success: 'PDF descargado con �xito', error: 'Error al generar el PDF' }); }}
                    ...
                    <div>
                        {inventoryItems.length === 0 ? (
                            <div className="bg-white rounded-2xl border border-gray-100 p-10 text-center text-gray-400">
                                ...
                                <p className="text-xs mt-1">Aparecerán aquí cuando se transfieran desde Bodega Ecuador</p>
                            </div>
                        ) : (
                ...
                {activeTab === 'vendido' && (
                    <div>
                        {soldItems.length === 0 ? (
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                {soldItems.map(item => (
                                    <ProductCard key={item.id} item={item} showReturnButton />
                                ))}
                            </div>
                        )}
                    </div>
            ...
            {showPayModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden z-10">
                        <div className="bg-pink-600 px-5 py-4 flex items-center justify-between text-white">
                        ...
                        <div className="p-5 space-y-4">
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full pl-8 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-xl font-bold text-gray-800 focus:ring-2 focus:ring-pink-500 focus:outline-none"
                                        placeholder="0.00"
                                        autoFocus
                                        min="0.01"
                                        step="0.01"
                            ...
                            <button
                                ...
                                className="w-full bg-pink-600 hover:bg-pink-700 disabled:bg-pink-300 text-white font-bold py-3 rounded-xl flex items-center justify-center space-x-2 transition-all active:scale-95"
                            >
                                {paying ? (
                    ...
                    <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden z-10">
                        ...
                        <div className="p-5 space-y-4">
                            ...
                            <button
                                ...
                                className="w-full bg-pink-600 hover:bg-pink-700 disabled:bg-pink-300 text-white font-bold py-3 rounded-xl transition-all"
                            >
