...
import { yuriInventoryService, YuriInventoryItem } from '../services/yuriInventoryService';
import { yuriPaymentService, YuriPayment } from '../services/yuriPaymentService';
...
const AppYuri: React.FC = () => {
    ...
    // Pagos
    const [payments, setPayments] = useState<YuriPayment[]>([]);
    ...
    // Edición de precio
    const [editingItem, setEditingItem] = useState<YuriInventoryItem | null>(null);
    ...
    const loadData = useCallback(async () => {
        try {
            ...
            const [items, pays] = await Promise.all([
                yuriInventoryService.getAll(),
                yuriPaymentService.getAll()
            ]);
            setAllItems(items);
    ...
    const handleMarkAsSold = async (item: YuriInventoryItem) => {
        try {
            await yuriInventoryService.markAsSold(item.id);
    ...
    const handleReturn = async (item: YuriInventoryItem) => {
        try {
            ...
            await yuriInventoryService.markAsReturned(item.id);
    ...
    const handlePay = async () => {
        ...
        setPaying(true);
        try {
            await yuriPaymentService.addPayment(amount);
    ...
    const handleUpdatePrice = async () => {
        ...
        try {
            // 1. Actualizar inventario de Yuri
            await yuriInventoryService.updatePrice(editingItem.id, price, editingItem.quantity);

            // 2. Actualizar notas de salida para que la deuda en el Dashboard sea correcta
            const allNotes = await exitNoteService.getAll();
            const yuriNotes = allNotes.filter(n =>
                n.sellerId === 'yuri' &&
    ...
    const ProductCard = ({ item, showSoldButton, showReturnButton }: { item: YuriInventoryItem; showSoldButton?: boolean; showReturnButton?: boolean }) => (
            ...
            <div className="p-3 flex flex-col flex-1">
                ...
                <div className="mt-auto pt-2 border-t border-gray-50 flex justify-between items-center">
                    <div className="flex flex-col">
                        ...
                        <span className="text-sm font-bold text-indigo-600">${item.unitPrice.toFixed(2)}</span>
                    </div>
                    {!isReadOnly && (
                        <button
                            onClick={() => {
    ...
    return (
        <div className="min-h-screen bg-gray-50 pb-10">
            {/* Header */}
            <div className="bg-gradient-to-r from-indigo-500 to-indigo-700 text-white px-4 py-5 shadow-lg">
                <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            <h1 className="text-xl font-bold">Yuri Marquez</h1>
                            <p className="text-indigo-100 text-sm flex items-center space-x-1">
                    ...
                    <div className="p-4">
                        {!isReadOnly && (
                            <button onClick={() => { setPayAmount(''); setShowPayModal(true); }} disabled={totalDebt <= 0}
                                className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-bold py-3 rounded-xl flex items-center justify-center space-x-2 transition-all active:scale-95">
                        ...
                        <button onClick={() => generateSellerAppInventoryPDF("Yuri Marquez", inventoryItems)}
                    ...
                    <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden z-10">
                        ...
                        <div className="p-5 space-y-4">
                            ...
                            <button
                                ...
                                className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-300 text-white font-bold py-3 rounded-xl flex items-center justify-center space-x-2 transition-all active:scale-95"
                            >
                                {paying ? (
