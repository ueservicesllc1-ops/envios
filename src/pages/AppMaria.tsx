...
const AppMaria: React.FC = () => {
    const navigate = useNavigate();
    const { user, loading: authLoading } = useAnonymousAuth();
    const [session, setSessionState] = useState<SellerSession | null>(null);
    const isReadOnly = !session?.isAdmin && !session?.isSuperAdmin && session?.id !== 'maria';

    const [loading, setLoading] = useState(true);
    const [allItems, setAllItems] = useState<MariaInventoryItem[]>([]);
    ...
    // Pagos
    const [payments, setPayments] = useState<MariaPayment[]>([]);
    ...
    // Edición de precio
    const [editingItem, setEditingItem] = useState<MariaInventoryItem | null>(null);
    ...
    const loadData = useCallback(async () => {
        try {
            ...
            const [items, pays] = await Promise.all([
                mariaInventoryService.getAll(),
                mariaPaymentService.getAll()
            ]);
            setAllItems(items);
    ...
    const handleMarkAsSold = async (item: MariaInventoryItem) => {
        try {
            await mariaInventoryService.markAsSold(item.id);
    ...
    const handleReturn = async (item: MariaInventoryItem) => {
        try {
            ...
            await mariaInventoryService.markAsReturned(item.id);
    ...
    const handlePay = async () => {
        ...
        setPaying(true);
        try {
            await mariaPaymentService.addPayment(amount);
    ...
    const handleUpdatePrice = async () => {
        ...
        try {
            // 1. Actualizar inventario de Maria
            await mariaInventoryService.updatePrice(editingItem.id, price, editingItem.quantity);

            // 2. Actualizar notas de salida para que la deuda en el Dashboard sea correcta
            const allNotes = await exitNoteService.getAll();
            const mariaNotes = allNotes.filter(n =>
                n.sellerId === 'maria' &&
    ...
    const ProductCard = ({ item, showSoldButton, showReturnButton }: { item: MariaInventoryItem; showSoldButton?: boolean; showReturnButton?: boolean }) => (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
            ...
            >
                ...
                <div className="absolute top-2 right-2 bg-teal-600 text-white text-xs font-bold px-2 py-1 rounded-full shadow">
                    x{item.quantity}
                </div>
            </div>
            ...
            <div className="p-3 flex flex-col flex-1">
                ...
                <div className="mt-auto pt-2 border-t border-gray-50 flex justify-between items-center">
                    <div className="flex flex-col">
                        ...
                        <span className="text-sm font-bold text-teal-600">${item.unitPrice.toFixed(2)}</span>
                    </div>
                    {!isReadOnly && (
                        <button
                            onClick={() => {
    ...
    return (
        <div className="min-h-screen bg-gray-50 pb-10">
            {/* Header */}
            <div className="bg-gradient-to-r from-teal-500 to-teal-700 text-white px-4 py-5 shadow-lg">
                <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            <h1 className="text-xl font-bold">Maria E. Castro</h1>
                            <p className="text-teal-100 text-sm flex items-center space-x-1">
                    ...
                    <div className="p-4">
                        {!isReadOnly && (
                            <button onClick={() => { setPayAmount(''); setShowPayModal(true); }} disabled={totalDebt <= 0}
                                className="w-full bg-teal-600 hover:bg-teal-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-bold py-3 rounded-xl flex items-center justify-center space-x-2 transition-all active:scale-95">
                    ...
                    <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden z-10">
                        ...
                        <div className="p-5 space-y-4">
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full pl-8 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-xl font-bold text-gray-800 focus:ring-2 focus:ring-teal-500 focus:outline-none"
                                        placeholder="0.00"
                                        autoFocus
                                        min="0.01"
                                        step="0.01"
                            ...
                            <button
                                ...
                                className="w-full bg-teal-600 hover:bg-teal-700 disabled:bg-teal-300 text-white font-bold py-3 rounded-xl flex items-center justify-center space-x-2 transition-all active:scale-95"
                            >
                                {paying ? (
                    ...
                    <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden z-10">
                        ...
                        <div className="p-5 space-y-4">
                            ...
                            <button
                                ...
                                className="w-full bg-teal-600 hover:bg-teal-700 disabled:bg-teal-300 text-white font-bold py-3 rounded-xl transition-all"
                            >
